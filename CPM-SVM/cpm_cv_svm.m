function [y_predict,pmask,mdl]=cpm_cv_svm(x,y,pthresh,kfolds)
...
for leftout = 1:kfolds
    ...
    [~, ~, pmask, mdl] = cpm_train_svm(x_train, y_train,pthresh);
    
    % Test Connectome-based Predictive Model
    [y_predict(testinds)] = cpm_test_svm(x_test,mdl,pmask);
end
